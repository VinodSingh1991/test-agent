...
class RAGQuery(BaseModel):
    ...
    entity: str = Field(description="CRM entity type (Lead, Opportunity, Account, Contact, Case, Task, Dashboard)")
    view_type: str = Field(description="View type (detail, list, form, dashboard, search)")
...
{format_instructions}"""),
    ...
    def invoke(self, analysis: Dict[str, Any]) -> RAGQuery:
        ...
        try:
            ...
            print(f"[QueryReformulator] Entity: {rag_query.entity}, View: {rag_query.view_type}")
            ...
            entity = "lead"
            for ent in ["lead", "opportunity", "account", "contact", "case", "task", "dashboard"]:
                if ent in intent.lower():
                    entity = ent
                    break
            
            # Simple view type detection
            view_type = "detail"
            if "list" in intent or "show_all" in intent:
                view_type = "list"
            ...
            search_query = f"{entity} {view_type} layout with {' and '.join(components)}"
            
            return RAGQuery(
                search_query=search_query,
                entity=entity,
                view_type=view_type,
                filters={"pattern": view_type},
                confidence=0.7
            )
