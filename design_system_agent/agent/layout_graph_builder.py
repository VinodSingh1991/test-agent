...
class GraphBuilder:
    ...
    def build(executor: WorkflowExecutor) -> StateGraph:
        ...
        Flow: plan → normalize → analyze → reformulate → retrieve(10→3) 
        ...
        workflow.add_node("analyze_query", executor.analyze_query)
        workflow.add_node("reformulate_query", executor.reformulate_query)
